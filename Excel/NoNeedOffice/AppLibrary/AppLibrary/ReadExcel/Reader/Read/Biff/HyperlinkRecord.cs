		...
		internal HyperlinkRecord(Record t, Sheet s, WorkbookSettings ws):base(t)
		{
			...
			if ((data[28] & 0x3) == 0x03)
			{
				...
				catch (Exception e)
				{
                    ...
                    AppLibrary.ReadExcel.CellReferenceHelper.getCellReference(firstColumn, firstRow, sb1);
                    AppLibrary.ReadExcel.CellReferenceHelper.getCellReference(lastColumn, lastRow, sb2);
					sb1.Insert(0, "Exception when parsing URL ");
					sb1.Append('\"').Append(sb2.ToString()).Append("\".  Using default.");
					logger.warn(sb1, e);
					url = new Uri("http://www.MDS.com.cn");
				}
			}
