...
namespace AppLibrary.Biff.Formula
{
	...
	class Attribute:Operator, ParsedThing
	{
		...
		public Attribute(StringFunction sf, WorkbookSettings ws)
		{
            ...
            if (sf.getFunction(settings) == AppLibrary.Biff.Formula.Function.SUM)
			{
				options |= sumMask;
			}
            else if (sf.getFunction(settings) == AppLibrary.Biff.Formula.Function.IF)
			{
				options |= ifMask;
			}
		}
		...
		public override void  getString(System.Text.StringBuilder buf)
		{
			...
			{
				ParseItem[] operands = getOperands();
                buf.Append(AppLibrary.Biff.Formula.Function.SUM.getName(settings));
				buf.Append('(');
			...
			{
                buf.Append(AppLibrary.Biff.Formula.Function.IF.getName(settings));
				buf.Append('(');
