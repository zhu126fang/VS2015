...
namespace AppLibrary.Biff.Formula
{
	...
	class CellReference:Operand, ParsedThing
	{
		...
		{
			column = AppLibrary.Biff.CellReferenceHelper.getColumn(s);
			row = AppLibrary.Biff.CellReferenceHelper.getRow(s);
			columnRelative = AppLibrary.Biff.CellReferenceHelper.isColumnRelative(s);
			rowRelative = AppLibrary.Biff.CellReferenceHelper.isRowRelative(s);
		}
		...
		{
			AppLibrary.Biff.CellReferenceHelper.getCellReference(column, !columnRelative, row, !rowRelative, buf);
		}
