...
namespace AppLibrary.Biff.Formula
{
	...
	class CellReference3d:Operand, ParsedThing
	{
		...
		public CellReference3d(string s, ExternalSheet w)
		{
			...
			column = AppLibrary.Biff.CellReferenceHelper.getColumn(cellString);
			row = AppLibrary.Biff.CellReferenceHelper.getRow(cellString);
		...
		{
			AppLibrary.Biff.CellReferenceHelper.getCellReference(sheet, column, !columnRelative, row, !rowRelative, workbook, buf);
		}
